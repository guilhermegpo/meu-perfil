 ...
 *
 * Saídas em `public/assets/`:
 *   profile/foto-oficial.png            original, preservado (pixels idênticos)
 *   profile/foto-{480,960}.{avif,webp,jpg}   retrato 4:5 para o card do Sobre
 *   profile/avatar-{96,192}.{avif,webp,jpg}  recorte quadrado do rosto
 *   projects/{app}-hero.png             original, preservado
...
if (await exists(photoSource)) {
  ...
  await emit(base.clone(), path.join(dir, 'foto-oficial.png'));
...
for (const [slug, file] of apps) {
  ...
  await emit(base.clone(), path.join(projectsDir, `${slug}-hero.png`));
